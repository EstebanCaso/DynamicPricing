  ...
  const fetchCompetitorData = async () => {
    try {
      ...
      if (competitors && competitors.length > 0) {
        ...
        if (supabaseData.length > 0) {
          // Use the average price directly (already in USD) without occupancy rate multiplication
          const userAvgPrice = supabaseData.reduce((sum, item) => sum + (item.price || 0), 0) / supabaseData.length;
          
          performanceData.push({
            hotel: userHotelName || "Our Hotel",
            revenue: Math.round(userAvgPrice) // Use price directly, not revenue
          });
        }
          ...
          try {
            ...
            if (currentRooms.length > 0) {
              const avgPrice = currentRooms.reduce((sum: number, room: any) => {
                const price = cleanPrice(room.price || room.rate || 0);
                return sum + price;
              ...
              if (avgPrice > 0) {
                ...
                console.log(`✅ Added competitor: ${competitor.nombre} - Avg Price: $${avgPrice}, Revenue: $${competitorRevenue}`);
              }
            } else {
                ...
                if (latestDate && roomsJson) {
                  ...
                  if (latestRooms.length > 0) {
                    ...
                    if (avgPrice > 0) {
                      ...
                      console.log(`✅ Added competitor (${latestDate}): ${competitor.nombre} - Avg Price: $${avgPrice}, Revenue: $${competitorRevenue}`);
                    }
                  }
                }
              }
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm font-medium text-emerald-600">
                      {(() => {
                        ...
                        const marketAvg = competitors.reduce((sum, item) => sum + item.revenue, 0) / competitors.length;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="backdrop-blur-xl bg-glass-100 border border-glass-200 rounded-2xl shadow-xl p-6 hover:shadow-2xl transition-all duration-300">
              ...
              <AreaChart data={(() => {
                if (supabaseData.length === 0) {
                  ...
                  for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
                    ...
                    // Generate realistic price data with seasonal and weekly patterns
                    const basePrice = 120;
                    const seasonalFactor = 1 + 0.3 * Math.sin((dayIndex / 90) * Math.PI * 2);
                    const weeklyFactor = 1 + 0.2 * Math.sin((dayIndex / 7) * Math.PI * 2);
                    const noise = (Math.random() - 0.5) * 0.1;
                    const price = Math.round(basePrice * seasonalFactor * weeklyFactor * (1 + noise));
                ...
                filteredData.forEach((item: any) => {
                  ...
                  // Clean and convert price from MXN to USD
                  const rawPrice = item.price;
                  if (!rawPrice) return;
                  
                  // Convert MXN to USD (approximate rate: 1 USD = 18.5 MXN)
                  const priceInUSD = cleanPrice(rawPrice) / 18.5;
                  ...
                  if (priceInUSD > 0) {
                    ...
                    dailyPrices[dateStr].total += priceInUSD;
                ...
                const historicalData = Object.entries(dailyPrices)
                  .map(([date, data]) => {
                    ...
                    return {
                      day: label,
                      price: Math.round(data.total / data.count * 100) / 100, // Round to 2 decimal places
                      date: date,
                      count: data.count,
                ...
                <Tooltip content={({ active, payload, label }) => {
                  ...
                  return (
                    <div className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm shadow-lg">
                      <div className="font-medium text-gray-900">{label}</div>
                      <div className="text-gray-700">Price: ${data.price.toFixed(2)} USD</div>
                ...
                <ReferenceLine y={(() => {
                  ...
                  const totalPrice = data.reduce((sum, item) => sum + (cleanPrice(item.price) / 18.5), 0);
                  return Math.round(totalPrice / data.length * 100) / 100;
                  ...
                  value: `Avg $${(() => {
                    ...
                    const totalPrice = data.reduce((sum, item) => sum + (cleanPrice(item.price) / 18.5), 0);
                    return (totalPrice / data.length).toFixed(2);
                  })()} USD`,
                  position: "right",
                  fill: "#64748b",
                  fontSize: 10,
          ...
          {(() => {
            ...
            // Convert prices to USD and filter valid prices
            const prices = filteredData
              .map(item => cleanPrice(item.price) / 18.5) // Convert MXN to USD
              .filter(price => price > 0);
              
            if (prices.length === 0) return null;
            
            const avgPrice = Math.round(prices.reduce((sum, price) => sum + price, 0) / prices.length * 100) / 100;
            const minPrice = Math.round(Math.min(...prices) * 100) / 100;
            const maxPrice = Math.round(Math.max(...prices) * 100) / 100;
            const priceRange = Math.round((maxPrice - minPrice) * 100) / 100;
              ...
              <div className="mt-4 pt-4 border-t border-gray-200">
                <div className="grid grid-cols-4 gap-4 text-center">
                  <div>
                    ...
                    <p className="text-lg font-semibold text-arkus-600">${avgPrice.toFixed(2)}</p>
                    <p className="text-xs text-gray-500">USD</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-600">Price Range</p>
                    <p className="text-lg font-semibold text-blue-600">${priceRange.toFixed(2)}</p>
                    <p className="text-xs text-gray-500">USD</p>
                  </div>
                  <div>
                ...
                <div className="mt-3 pt-3 border-t border-gray-100">
                  <div className="text-center">
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      Prices converted from MXN to USD (1 USD ≈ 18.5 MXN)
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="backdrop-blur-xl bg-glass-100 border border-glass-200 rounded-2xl shadow-xl p-6 hover:shadow-2xl transition-all duration-300">
            ...
            {revenuePerformanceData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart data={(() => {
                  ...
                  const marketAvgRevenue = competitors.length > 0 
                    ? competitors.reduce((sum, item) => sum + item.revenue, 0) / competitors.length 
                    : ourHotel.revenue;
                  
                  const marketAvgPrice = competitors.length > 0 
                    ? competitors.reduce((sum, item) => sum + (item.revenue / 0.80), 0) / competitors.length 
                  ...
                  const radarData = [
                    ...
                    {
                      metric: "Market Share",
                      ourHotel: Math.min(100, (ourHotel.revenue / (ourHotel.revenue + competitors.reduce((sum, item) => sum + item.revenue, 0))) * 200),
                      marketAvg: 100,
                      fullMark: 120
                    },
                    {
          ...
          {revenuePerformanceData.length > 0 && (() => {
            ...
            if (!ourHotel || competitors.length === 0) return null;
            
            const marketAvgRevenue = competitors.reduce((sum, item) => sum + item.revenue, 0) / competitors.length;
                ...
                <span className="text-xs text-blue-600 bg-blue-50 px-2 py-1 rounded-lg">
                  {(() => {
                    ...
                    const validPrices = filteredData
                      .map(item => cleanPrice(item.price) / 18.5) // Convert MXN to USD (same as Historical Prices)
                      .filter(price => price > 0);
                    
                    if (validPrices.length === 0) return "No valid prices";
          ...
          <div className="h-80">
            {(() => {
              ...
              filteredData.forEach((item: any) => {
                ...
                // Clean and convert price from MXN to USD (same as Historical Prices)
                const rawPrice = item.price;
                if (!rawPrice) return;
                    ...
                    <Tooltip 
                      content={({ active, payload, label }) => {
                        ...
                        return (
                          <div className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm shadow-lg">
                            <div className="font-medium text-gray-900">{label}</div>
                            <div className="text-gray-700">Our Price: ${data.ours}</div>
                            <div className="text-gray-600">Market Estimate: ${data.marketAvg}</div>
                            <div className={`font-medium ${data.gap >= 0 ? 'text-red-600' : 'text-green-600'}`}>
                              Gap: {data.gap >= 0 ? '+' : ''}${data.gap} ({data.gapPercent}%)
                            </div>
              ...
              <div className="mt-6 pt-4 border-t border-gray-200">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                  <div>
                    ...
                    <p className="text-lg font-semibold text-arkus-600">
                      ${ourAvgPrice.toFixed(2)}
                    </p>
                    <p className="text-xs text-gray-500">USD per night</p>
                  </div>
                  
                  <div>
                    <p className="text-xs text-gray-600">Market Estimate</p>
                    <p className="text-lg font-semibold text-blue-600">
                      ${marketEstimate.toFixed(2)}
                    </p>
                    <p className="text-xs text-gray-500">USD per night</p>
