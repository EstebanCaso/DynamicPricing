...
• Your Average Price: ${data.currency === 'USD' ? '$' : '$'}${data.marketPosition.yourPrice.toLocaleString()}
• Market Average: ${data.currency === 'USD' ? '$' : '$'}${data.marketPosition.marketAverage.toLocaleString()}
...
• Total data points: ${data.historicalData.length}
• Date range: ${data.historicalData[0]?.date || 'N/A'} to ${data.historicalData[data.historicalData.length - 1]?.date || 'N/A'}
• Price range: ${data.currency === 'USD' ? '$' : '$'}${Math.min(...data.historicalData.map(d => d.price)).toLocaleString()} - ${data.currency === 'USD' ? '$' : '$'}${Math.max(...data.historicalData.map(d => d.price)).toLocaleString()}
• Average bookings per day: ${(data.historicalData.reduce((sum, d) => sum + d.count, 0) / data.historicalData.length).toFixed(1)}
