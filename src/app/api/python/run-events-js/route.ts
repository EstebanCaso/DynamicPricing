...
export async function POST(request: NextRequest) {
	try {
		...
		// Use only Songkick scraper for now
		const scriptPath = path.join(process.cwd(), 'scripts', 'scrape_songkick.js')
		if (!fs.existsSync(scriptPath)) {
			console.error('[run-events-js] Script not found at path:', scriptPath)
			return NextResponse.json({ success: false, error: 'Script file not found' }, { status: 500 })
		}
		const args: string[] = []
		if (latitude && longitude) {
			args.push(latitude.toString(), longitude.toString(), radius.toString())
		}
		...
		return await new Promise((resolve) => {
			...
			child.stdout.on('data', (data) => {
				...
				stdout += out
				console.log('[run-events-js][stdout]', out)
			})

			child.stderr.on('data', (data) => {
				const err = data.toString()
				stderr += err
				console.error('[run-events-js][stderr]', err)
			})

			child.on('close', async (code) => {
				console.log('[run-events-js] Process exited with code:', code)
				if (code === 0) {
					try {
						...
						let inserted = 0
						console.log('[run-events-js] Events to save:', events.length, 'User:', userUuid, 'Hotel:', hotelName)
						if (SUPABASE_URL && SUPABASE_ANON_KEY && userUuid) {
							...
							console.log('[run-events-js] Mapped events:', mapped.length, 'Deduplicated:', deduplicatedEvents.length, 'Sample:', deduplicatedEvents[0])
							...
							for (let i = 0; i < deduplicatedEvents.length; i += chunkSize) {
								...
								try {
									...
									if (res.ok) {
										...
										console.error('[run-events-js] Supabase insert failed:', res.status, errorText)
									}
								} catch (e) {
									console.error('[run-events-js] Supabase insert error:', (e as Error)?.message)
								}
							}
						}

						console.log('[run-events-js] Final result - inserted:', inserted, 'total events:', events.length)
						resolve(NextResponse.json({ success: true, inserted, count: Array.isArray(events) ? events.length : 0 }))
					} catch (err) {
						console.error('[run-events-js] Failed to parse stdout or save:', (err as Error)?.message)
